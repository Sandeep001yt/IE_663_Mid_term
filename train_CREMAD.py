...
def train_audio_video(epoch, train_loader, model, optimizer, merge_alpha=0.5, lambda_sf=0.5):
    model.train()
    # ----- RECORD LOSS AND ACC -----
    tl = Averager()
    ta = Averager()
    ...
    for step, (spectrogram, image, y) in enumerate(train_loader):
        ...
        if add_fea is None:
            ...
            loss_a = criterion(out_a, y).mean() + criterion(o_fea, y).mean() + criterion(add_fea, y).mean() - lambda_sf * criterion(add_fea, kl).mean() # criterion(add_fea, y-lambda_sf*kl).mean() = criterion(add_fea, y).mean() - lambda_sf * criterion(add_fea, kl).mean()
        ...
        if add_fea is None:
            ...
            kl = y*o_fea.detach().softmax(1)
            loss_v = criterion(out_v, y).mean() + criterion(o_fea, y).mean() + criterion(add_fea, y).mean() - lambda_sf * criterion(add_fea, kl).mean() # criterion(add_fea, y-lambda_sf*kl).mean() = criterion(add_fea, y).mean() - lambda_sf * criterion(add_fea, kl).mean()
    ...
    return model, ratio_a, loss_ave, loss_a_ave, loss_v_ave
...
def val(epoch, val_loader, model, merge_alpha=0.5):
    ...
    # return acc
    return f1, acc, mAP, f1_a, acc_a, mAP_a, f1_v, acc_v, mAP_v
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_sf', default = 0.5, type=float, help='Smoothing Factor')
    args = parser.parse_args()
    cfg = config

    # -----Save Training Logs-----
    model_folder_path = f"CREMAD_Models_lam{args.lam}"
    os.makedirs(model_folder_path)

    log_file = open(f"training_log/CREMAD_lam{args.lam}.csv", "w", newline="")
    ...
    for epoch in range(cfg['train']['epoch_dict']):
        ...
        scheduler.step()
        # model, ratio_a = train_audio_video(epoch, train_loader, model, optimizer,args.merge_alpha, args.lambda_sf)
        # acc = val(epoch, test_loader, model, args.merge_alpha)
        model, ratio_a, loss, loss_a, loss_v = train_audio_video(
            epoch, train_loader, model, optimizer, args.merge_alpha, args.lambda_sf
        )

        f1, acc, mAP, f1_a, acc_a, mAP_a, f1_v, acc_v, mAP_v = val(
            epoch, test_loader, model, args.merge_alpha
        )
        log_writer.writerow([
            epoch,
            loss,
